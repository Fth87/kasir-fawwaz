...
export function InventoryCombobox({ value, onSelect, isLoading = false }: InventoryComboboxProps) {
  ...
  const handleAddNewItem = async (data: NewItemFormValues) => {
    ...
    if (newItem) {
      // Otomatis pilih barang yang baru dibuat
      onSelect({ name: newItem.name, price: newItem.sellingPrice });
      setDialogOpen(false);
      setOpen(false);
      form.reset();
      setSearchQuery("");
      ...
      <DialogContent className="sm:max-w-lg">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleAddNewItem)} className="space-y-4 py-4">
            ...
            <FormField control={form.control} name="sku" render={({ field }) => ( <FormItem><FormLabel>SKU (Opsional)</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem> )} />
            <DialogFooter>
